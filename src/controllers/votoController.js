...
export const crearVoto = async (req, res) => {
    try {
        ...
        if (!propuestaId) {
            return res.status(400).json(ResponseFactory.badRequest('ID de propuesta es requerido'))
        }
        ...
        if (!estadosVotables.includes(propuesta.estado)) {
            return res.status(400).json(ResponseFactory.badRequest(
                `No se puede votar en propuestas con estado "${propuesta.estado}". Solo se permite votar en propuestas Pendientes o En Revisión.`
            ))
        }
        ...
        res.status(201).json(ResponseFactory.success(votoGuardado, 'Voto registrado exitosamente'))

    } catch (error) {
        // Error de voto duplicado (código 11000 de MongoDB)
        if (error.code === 11000) {
            return res.status(400).json(ResponseFactory.badRequest('Ya has votado en esta propuesta'))
        }
        ...
        res.status(500).json(ResponseFactory.internalError('Error al registrar el voto', error.message))
    }
}
...
export const getVotosPorPropuesta = async (req, res) => {
    try {
        ...
        res.status(200).json(ResponseFactory.successWithCount(votos))

    } catch (error) {
        res.status(500).json(ResponseFactory.internalError('Error al obtener los votos', error.message))
    }
}
...
export const getEstadisticasVotos = async (req, res) => {
    try {
        ...
        res.status(500).json(ResponseFactory.internalError('Error al obtener estadísticas', error.message))
    }
}
...
export const verificarVotoUsuario = async (req, res) => {
    try {
        ...
        res.status(500).json(ResponseFactory.internalError('Error al verificar el voto', error.message))
    }
}
...
export const getVotosUsuario = async (req, res) => {
    try {
        ...
        res.status(500).json(ResponseFactory.internalError('Error al obtener el historial de votos', error.message))
    }
}
